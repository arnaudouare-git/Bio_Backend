...
public class CommandeService {
    ...
    private static final Set<String> STATUTS_VALIDES = Set.of(
            "EN_ATTENTE", "CONFIRMEE", "EN_PREPARATION", "EXPEDIEE", "LIVREE", "ANNULEE", "REFUSEE");
    ...
    /**
     * Delai de retractation (ajoute le 2026-09-24, retour client en entretien) :
     * un acheteur ne peut annuler que dans les 12h suivant la commande, EN PLUS
     * de la regle deja existante (aucun producteur n'a encore commence a
     * traiter sa ligne) -- les deux conditions doivent etre vraies.
     */
    private static final long DELAI_RETRACTATION_HEURES = 12;
    ...
    public CommandeResponse passerCommande(CreerCommandeRequest requete, Utilisateur demandeur) {
        ...
        for (LigneCommandeRequest ligneRequete : requete.getLignes()) {
            ...
            // Ajoute le 2026-09-24 : un Producteur ne peut pas acheter son propre
            // produit (dette notee dans Module_Commandes_2026-09-23.md). L'acheteur
            // et le Producteur sont tous deux types Utilisateur (heritage JOINED),
            // donc comparer les id suffit, quel que soit le role reel de l'acheteur.
            if (produit.getProducteur().getId().equals(acheteur.getId())) {
                throw new IllegalArgumentException(
                        "Impossible d'acheter votre propre produit : \"" + produit.getNom() + "\".");
            }
        ...
        // Ajoute le 2026-09-24 : notifie chaque Producteur concerne (une commande
        // peut porter sur plusieurs Producteurs differents) de la reception de
        // cette nouvelle commande -- dette notee dans Module_Commandes_2026-09-23.md.
        notificationService.notifierReceptionCommande(enregistree);
    ...
    public CommandeResponse changerStatut(Long commandeId, ChangerStatutCommandeRequest requete, Utilisateur demandeur) {
        ...
        if (estAdmin) {
            ...
            java.time.LocalDateTime limiteRetractation =
                    commande.getDateCommande().plusHours(DELAI_RETRACTATION_HEURES);
            if (java.time.LocalDateTime.now().isAfter(limiteRetractation)) {
                throw new IllegalArgumentException(
                        "Cette commande ne peut plus etre annulee : le delai de retractation de "
                                + DELAI_RETRACTATION_HEURES + "h (a compter de la commande, le "
                                + commande.getDateCommande() + ") est depasse.");
            }
     ...
     *
     * Depuis le 2026-09-24 : un statut "REFUSEE" (avec motif obligatoire,
     * voir ChangerStatutCommandeRequest.motif) permet a un Producteur de
     * refuser sa ligne tant qu'elle est encore EN_ATTENTE -- le stock est
     * alors restitue automatiquement.
     */
    @Transactional
    public CommandeResponse changerStatutLigne(Long commandeId, Long ligneId,
        ...
        // Module Refus de commande (ajoute le 2026-09-24, priorite Tier 1) :
        // cas a part, traite AVANT le controle de progression ORDRE_STATUTS
        // ci-dessous (REFUSEE n'y figure pas -- c'est une sortie, pas une
        // etape). Un Producteur ne peut refuser que tant que sa ligne est
        // encore EN_ATTENTE (il ne s'est pas encore engage a la traiter) ;
        // un Administrateur peut forcer un refus a tout moment. Dans les
        // deux cas, le stock decremente au moment de la commande est restitue.
        if ("REFUSEE".equals(nouveauStatut)) {
            if (requete.getMotif() == null || requete.getMotif().isBlank()) {
                throw new IllegalArgumentException(
                        "Un motif est obligatoire pour refuser une ligne de commande.");
            }
            if ("REFUSEE".equals(ligne.getStatut())) {
                throw new IllegalArgumentException("Cette ligne a deja ete refusee.");
            }
            if (!estAdmin && !"EN_ATTENTE".equals(ligne.getStatut())) {
                throw new IllegalArgumentException(
                        "Impossible de refuser cette ligne : elle a deja ete confirmee ou traitee "
                                + "(statut actuel : \"" + ligne.getStatut() + "\"). Seule une ligne "
                                + "EN_ATTENTE peut etre refusee (un Administrateur peut forcer un refus "
                                + "a tout moment).");
            }
            ...
            ligne.setMotifRefus(requete.getMotif());
            ligne.setStatut("REFUSEE");
            Commande enregistree = commandeRepository.save(commande);
            return versReponse(enregistree);
        }
