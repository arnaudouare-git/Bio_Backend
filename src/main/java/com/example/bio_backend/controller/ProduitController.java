 ...
 *
 * GET http://localhost:8080/api/produits/proximite?lat=12.3714&lon=-1.5197&rayonKm=25
 * -> catalogue filtre par distance (Haversine) au point (lat, lon) donne,
 *    trie du plus proche au plus loin ; rayonKm optionnel (defaut 50).
 *    Ajoute le 2026-09-24 (module Recherche geolocalisee, priorite Tier 1).
 *
...
public class ProduitController {
    ...
    public ResponseEntity<List<ProduitResponse>> rechercherProximite(
            @RequestParam double lat,
            @RequestParam double lon,
            @RequestParam(defaultValue = "50") double rayonKm) {
        return ResponseEntity.ok(produitService.rechercherProduitsProximite(lat, lon, rayonKm));
    }
